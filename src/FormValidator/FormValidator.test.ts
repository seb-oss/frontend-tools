...
describe("Form Validator", () => {
    ...
    describe("Testing each validation type individually", () => {
        const testCases: Array<TestCase1> = [
            ...
            { type: "required", value: 2, expected: noError },
            {
                type: "isDate",
                value: "abc",
                expected: { errorCode: "invalidDate" },
            },
            { type: "isDate", value: now, expected: noError },
            {
                type: "dateRange",
                value: modifyDate(now, 1, "SUBTRACT", "year"),
                specs: { minDate: now },
                expected: {
            ...
            {
                type: "dateRange",
                value: modifyDate(now, 1, "ADD", "year"),
                specs: { maxDate: now },
                expected: {
            ...
            },
            {
                ...
                value: now,
                specs: {
                    minDate: modifyDate(now, 1, "SUBTRACT", "day"),
                    maxDate: modifyDate(now, 1, "ADD", "day"),
                },
                expected: noError,
            },
            {
            ...
            },
            {
                type: "validEmail",
                value: "abc@email",
                expected: { errorCode: "invalidEmail" },
            },
            {
                type: "validEmail",
                value: "abcdefg",
            ...
            { type: "validEmail", value: "abc@email.com", expected: noError },
            {
                type: "strongPassword",
                value: "123",
                expected: { errorCode: "weakPassword" },
            },
            {
                type: "strongPassword",
                value: "RYGc1tc7C6nyjzz",
                expected: noError,
            },
            {
                type: "isPhoneNumber",
                value: "546",
                expected: { errorCode: "invalidPhoneNumber" },
            },
            {
                type: "isPhoneNumber",
            ...
            { type: "validEmail", value: null, expected: noError },
            { type: "strongPassword", value: null, expected: noError },
            { type: "isPhoneNumber", value: null, expected: noError },
        ];

        testCases.map((testCase: TestCase1) => {
            let title: string = `- ${testCase.type}`;
            title += ` | Value(${
                Object(testCase.value) !== testCase.value
                    ? testCase.value
                    : JSON.stringify(testCase.value)
            })`;
            title += !isEmpty(testCase.specs)
                ? ` | Specs(${JSON.stringify(testCase.specs)})`
                : "";
            title += ` | Expected Error(${JSON.stringify(testCase.expected)})`;
            test(title, () => {
                const validator: FormValidator<CustomType> =
                    new FormValidator<CustomType>({ typeAny: testCase.value })
                        .addValidation(
                            ["typeAny"],
                            testCase.type as any,
                            testCase.specs as any
                        )
                        .validate();
                const expectedErrors: ModelErrors<CustomType> = {
                    typeAny: testCase.expected,
                };
                if (testCase.expected) {
                    expect(validator.getErrors()).toMatchObject(expectedErrors);
                } else {
                    expect(Object.keys(validator.getErrors()).length).toEqual(
                        0
                    ); // no errors
                }
            });
    ...
    it("Should allow passing a custom validator", () => {
        const expectedErrors: ModelErrors<CustomType> = {
            first: { errorCode: "moreThanMaxValue" },
        };
        const test: CustomType = { first: 123, second: 100 };
        const validator: FormValidator<CustomType> =
            new FormValidator<CustomType>(test)
    ...
    it("Should run normal validations before custom validation", () => {
        const expectedNormalErrors: ModelErrors<CustomType> = {
            first: { errorCode: "lessThanMinValue" },
        };
        const expectedCustomErrors: ModelErrors<CustomType> = {
            first: { errorCode: "moreThanMaxValue" },
        };
        const test: CustomType = { first: 123, second: 100 };
        const validator: FormValidator<CustomType> =
    ...
    describe("Should not add validation if wrong parameters are passed", () => {
        ...
        testCases.map((testCase: TestCase2) => {
            test(testCase.title, () => {
                const validator: FormValidator<CustomType> =
                    new FormValidator<CustomType>({ first: 123, second: 100 })
    ...
    describe("Should not add custom validation if wrong parameters are passed", () => {
        ...
        testCases.map((testCase: TestCase2) => {
            test(testCase.title, () => {
                const validator: FormValidator<CustomType> =
                    new FormValidator<CustomType>({ first: 123, second: 100 })
                        .addCustomValidation(testCase.fields, () => ({
                            errorCode: "afterMaxDate",
                        }))
                        .validate();
                expect(validator.getErrors()).toEqual({});
    ...
    describe("Should not store the form model if it's invalid", () => {
        test("Empty object", () => {
            expect(
                new FormValidator<CustomType>({}).validate().getErrors()
            ).toMatchObject({});
        });
        test("Non object", () => {
            expect(
                new FormValidator<CustomType>([] as any).validate().getErrors()
            ).toMatchObject({});
        });
    });
});
