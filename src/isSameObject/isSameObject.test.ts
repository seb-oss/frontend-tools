...
describe("Util: isSameObject", () => {
    const testCases: Array<ComparisonTestCase> = [
        ...
        {
            statement:
                "should return false for different objects for deep comparison",
            objectA: { atr: 1, atr2: 123 },
            objectB: { atr: 1, atr2: "123" },
            isDeep: true,
            result: false,
        ...
        {
            statement:
                "should return false for same object for shallow comparison when attribute is not in order",
        ...
        {
            statement:
                "should return true for same object for shallow comparison when objects are exactly the same",
            objectA: { atr: 1, atr2: { level2a: 1, level2b: { level3: 123 } } },
    ...
    testCases.map((testCase: ComparisonTestCase) => {
        test(`- Statement: ${testCase.statement} | objectA: ${JSON.stringify(
            testCase.objectA
        )} | objectB: ${JSON.stringify(testCase.objectB)}`, () => {
            expect(
                isSameObject(
                    testCase.objectA,
                    testCase.objectB,
